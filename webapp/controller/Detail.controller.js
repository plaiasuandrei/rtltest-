...
sap.ui.define([
...
], function(BaseController, JSONModel, Dialog, formatter, Filter, TextArea, Button, MessageToast, FilterOperator) {
	"use strict";
	return BaseController.extend("rtl.controller.Detail", {
		...
		onTabSelect: function(oEvent) {
			...
			//depending on the tab the visbility of the submit and reject buttons is chnaged
			//the same is valid for the ObjectList header item
			switch (sKey) {
				case "info":
					oViewModel.setProperty("/isDetailObjectHeaderVisisble", true);
					oViewModel.setProperty("/isSubmitButtonVisible", true);
					oViewModel.setProperty("/isRejectButtonVisible", true);
					break;
				case "attach":
					oViewModel.setProperty("/isDetailObjectHeaderVisisble", true);
					oViewModel.setProperty("/isSubmitButtonVisible", false);
					oViewModel.setProperty("/isRejectButtonVisible", false);
					break;
				case "searchPO":
		...
		_onBindingChange: function() {
			...
			var sPath = oElementBinding.getPath(),
				...
				sCompanyCode = oObject.Bukrs,
				sVendorId = oObject.RdVendor,
				sPO = oObject.Ebeln,
			...
			oModel.setProperty("/showRejectionTab", false);
			// reset PO number search
			var aFilters = [];
			aFilters.push(new sap.ui.model.Filter("BUKRS", sap.ui.model.FilterOperator.EQ, sCompanyCode));
			aFilters.push(new sap.ui.model.Filter("RDVENDOR", sap.ui.model.FilterOperator.EQ, sVendorId));
			// 			var oBinding = 
			var oList = oView.byId("POItemList");
			oList.getBinding("items").filter(aFilters);
			//reset info tab
			oModel.setProperty("/selectedPOPath", "");
			var oListInfoTab = this.getView().byId("selectedPOInfoTabList");
			oListInfoTab.unbindAggregation("items", true);
			// 			//the info tab
			// 			oList = oView.byId("infoPOList");
			// 			if (sPO === "") {
			// 			    oViewModel =
			// 				sPO = '9999999999';
			// 			}
			...
			// 			oList.getBinding("items").filter(aFilters);
			oViewModel.setProperty("/poListBusy", false); // 			var oTable = oView.byId("ocrInvoiceLTable");
		...
		onPOSearch: function(oEvent) {
			...
			if (sQuery) {
				var oFilter = new Filter("SEARCH", FilterOperator.Contains, sQuery);
			}
		...
		rejectionConfirmed: function(text) {
			var oModel = this.getModel("detailView");
		},
		onRejectionSubmitDialog: function() {
			// 			var that = this;
			var dialog = new Dialog({
				...
				beginButton: new Button({
					...
					press: function() {
						...
						dialog.close(); // 		that.RejectionConfirmed(sText);
					}
				}),
			...
			// 					press: function() {
			// 						var sText = sap.ui.getCore().byId('submitDialogTextarea').getValue();
			// 						MessageToast.show('Rejected with note: ' + sText );
			// 						dialog.close();
			// 					} 
			// 				}),
			// 				endButton: new Button({
		...
		onPOSelected: function(oEvent) {
			var sPath = oEvent.getParameter("listItem").getBindingContext().getPath();
			sPath = sPath + "/EBELN";
			var oModel = this.getModel();
			var sPO = oModel.getProperty(sPath);
			this.getModel("detailView").setProperty("/selectedPOPath", sPath);
			//  MessageToast.show('tem  ' + sPath );
			var oList = this.getView().byId("selectedPOInfoTabList");
			//  var oTemplate = oList.getAggregation("items")[0];
			var oTemplate = this.byId("POITEM");
			oList.bindAggregation("items", "/searchPOSet", oTemplate);
			var aFilters = [];
			aFilters.push(new sap.ui.model.Filter("EBELN", sap.ui.model.FilterOperator.EQ, sPO));
			oList.getBinding("items").filter(aFilters);
		},
		...
		onValidate: function() {
			...
			// scenario 1: the dcoument has no PO and the user has selected a PO
			//     -> update the header field RD_PO_NUMBER with the selected PO
			//     -> the back end will take care of entering the correct lines in the OCRInvoiceL entity

			var oModel = this.getModel();
			try {
				var sModelPath = this.getView().getBindingContext().getPath();
				var oObject = oModel.getObject(sModelPath);
				var sPO = oObject.Ebeln;
				var sSelectedSPO = oModel.getProperty(this.getModel("detailView").getProperty("/selectedPOPath"));
				if (sSelectedSPO && !sPO) {
					// PO not existing on header line but chosen by user
					var oData = {
						Ebeln: sSelectedSPO
					};
					oModel.update(sModelPath, oData, {
