			...
			onTheMasterPage : {
				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("Document") === oObject.Document;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].Document;
										} else {
											sCurrentId = aItemsNotInTheList[0].Document;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ocrInvoiceHSet", {
											Document : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("Document"),
							title: oBindingContext.getProperty("RdVendorName")
						};
					}
				},
				...
				assertions : {
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("RdVendorName");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all ocrInvoiceHSet to be able to check for the total amount
